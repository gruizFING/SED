...
void ClienteFeeder::eventRoutine(Entity* who) {
	...
	b.schedule(b.tasaArribos, nuevoCliente, clienteF); //1 cada minuto
}
...
void elegirCajaCliente::eventRoutine(Entity* who) {
    ...
    if (caja->disponible) {
        ...
        b.schedule(b.tiempoServicioCliente, cliente, salidaC);
    }
    else {
...
void SalidaCliente::eventRoutine(Entity* who) {
    ...
    if (!b.filasCajas[cliente->cajaElegida]->empty()) {
        ...
        b.schedule(b.tiempoServicioCliente, nuevoClienteCaja, salidaC);
    }
    else {
