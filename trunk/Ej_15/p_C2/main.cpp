...
const unsigned int repeticiones = 50;
...
int main () {
    ...
    double tiempoEntreArribos = 1/maxRateArribos;
    bool cajasSoloRetiro[4] = {}; // No se toman en cuenta distintos tipos de cajas en este modelo
    ...
    for (unsigned int i = 1; i <= repeticiones; i++) {
        ...
        if (i == repeticiones) {
            ...
            b.lColaCajasTW.print(50, "largoColaCajasTW.txt");
            ...
            b.lColaCajasMeanTS.table(cantCellsTS, "largoColaCajasMeanTS.txt");
        }
    }
    ...
    cout << "Desviacion s para los largos de las colas: " << s_largoColas << endl << endl;
     ...
     // Calculo de intervalos de confianza para las medias con n=50
    cout << "Intervalo al 95% para los tiempos de espera" << endl;
    cout << x_espera - 2.0086 * (s_espera / sqrt(repeticiones)) << " < Mu < " << x_espera + 2.0086 * (s_espera / sqrt(repeticiones)) << endl;
    cout << "Intervalo al 99% para los tiempos de espera"<< endl;
    cout << x_espera - 2.6778 * (s_espera / sqrt(repeticiones)) << " < Mu < " << x_espera + 2.6778 * (s_espera / sqrt(repeticiones)) << endl << endl;

    cout << "Intervalo al 95% para la cantidad de clientes" << endl;
    cout << x_cantClientes - 2.0086 * (s_cantClientes / sqrt(repeticiones)) << " < Mu < " << x_cantClientes + 2.0086 * (s_cantClientes / sqrt(repeticiones)) << endl;
    cout << "Intervalo al 99% para la cantidad de clientes"<< endl;
    cout << x_cantClientes - 2.4033 * (s_cantClientes / sqrt(repeticiones)) << " < Mu < " << x_cantClientes + 2.6778 * (s_cantClientes / sqrt(repeticiones)) << endl << endl;

    cout << "Intervalo al 95% para el largo de las cola" << endl;
    cout << x_largoColas - 2.0086 * (s_largoColas / sqrt(repeticiones)) << " < Mu < " << x_largoColas + 2.0086 * (s_largoColas / sqrt(repeticiones)) << endl;
    cout << "Intervalo al 99% para el largo de las cola"<< endl;
    cout << x_largoColas - 2.6778 * (s_largoColas / sqrt(repeticiones)) << " < Mu < " << x_largoColas + 2.6778 * (s_largoColas / sqrt(repeticiones)) << endl << endl;
