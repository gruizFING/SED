...
// se contruyen los eventos B, los eventos C, las distribuciones, los recursos y los histogramas
								...
								lColaCajasTW("Media de largo de colas TW", *this)
    {
        ...
        for (unsigned int i = 0; i < cantC; i++) {
            ...
            pair<double, double> positionFila(xCajas - distCajaFila, yCajasInicial + i*distEntreCajas);
            filasEspera.push_back(positionFila);
            pair<double, double> position(xCajas, yCajasInicial + i*distEntreCajas);
            Caja* caja = new Caja(this->disp, position, cajasSoloRetiro[i]);
...
void Banco::doInitialSchedules() {
    ...
    TextSprite* largos = new TextSprite(this->disp, "Largo promedio de filas: ", 15, salidaDatos.first + 400, salidaDatos.second);
    largos->setVisible(true);
    TextSprite* maxFilaLargo = new TextSprite(this->disp, "Largo maximo de filas: ", 15, salidaDatos.first + 400, salidaDatos.second + 25);
